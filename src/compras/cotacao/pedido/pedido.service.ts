...
export class PedidoService {
  ...
  async gerarPdfPedidoExpress(res: ExpressResponse, id: string) {
    ...
    // Avança Y só pelo bloco esquerdo (metadados removidos)
    y = yLeft + 12;

    // ====== Tabela ======
    // Removemos 'Código' e colocamos 'Ref' como 1ª coluna
    const col: Record<string, number> = {
      ref: 70,           // agora a 1ª coluna
      descricao: 300,    // aumentei um pouco para compensar a remoção do código
      marca: 60,
      un: 20,
      qtd: 25,
      unit: 65,
    ...
    // Ajuste automático para caber na largura útil
    ...
    headers.forEach((h, i) => {
      ...
      doc.text(h, x, y, { width: w, align: i >= 4 ? 'right' : 'left' }); // alinha Qtd/Unit/Total à direita
      x += w;
    });
    y += 12;
    doc
    ...
    for (const it of pedido.itens) {
      ...
      // Agora a ordem das colunas é: Ref, Descrição, Marca, Un, Qtd, Unit, Total
      const row = [
        clamp(it.referencia ?? '', 18),         // Ref à esquerda
        clamp(it.pro_descricao, 50),            // Descrição
        clamp(it.mar_descricao ?? '', 15),      // Marca
        clamp(it.unidade ?? '', 6),             // Un
        fmtQtd(qtd),                            // Qtd
        unit ? fmtNum2(unit) : '',              // Unit (sem "R$")
        fmtNum2(linha),                         // Total (sem "R$")
      ];
